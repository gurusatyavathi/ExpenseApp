...
public class TransactionDaoImpl implements TransactionDao{
    ...
    public List<Transaction> getListOfTransactions(DBHelper dbHelper, long userID){
        ...
        // Query to retrieve transactions for the given user ID that are not deleted
        String query = "SELECT * FROM " + DBHelper.TABLE_TRANSACTIONS + " WHERE " + DBHelper.KEY_USER_ID + "=" + userID + " AND "+ DBHelper.KEY_DELETE + "=" + 0;
        // Execute the query
    ...
    public List<Transaction> getListOfDeletedTransactions(DBHelper dbHelper, long userID) {
        ...
        // Query to retrieve deleted transactions for the given user ID
        String query = "SELECT * FROM " + DBHelper.TABLE_TRANSACTIONS + " WHERE " + DBHelper.KEY_USER_ID + "=" + userID + " AND "+ DBHelper.KEY_DELETE + "=" + 1;
        // execute
        Cursor cursor = db.rawQuery(query, null);
